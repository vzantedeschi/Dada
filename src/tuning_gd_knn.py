...
results = {}
results = results.fromkeys(itertools.product(MU_LIST, BETA_LIST, STEP_LIST, NB_NEIGH_LIST), 0.)
    ...
    for mu in MU_LIST:
        ...
        for beta in BETA_LIST:
            ...
            for step in STEP_LIST:
                ...
                for k in NB_NEIGH_LIST:

                    print(mu, beta, step, k)

                    nodes_copy = deepcopy(nodes)
                    r = gd_reg_local_FW(nodes_copy, base_clfs, gd_method={"name":"knn", "pace_gd": step, "args":(k)}, beta=beta, mu=mu, nb_iter=NB_ITER, reset_step=False, callbacks=callbacks)

                    # keep value of last iteration
                    results[(mu, beta, step, k)] += r[NB_ITER]["accuracy"][1]


print("best mu, beta, step, k:", max(results, key=results.get))